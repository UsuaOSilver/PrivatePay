      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className="text-xs text-[var(--secondary)]">{wallets.length} wallet{wallets.length !== 1 ? 's' : ''}</p>
          </div>
        </div>
        <div className="text-2xl text-[var(--accent)] transition-transform" style={{ transform: expanded ? 'rotate(180deg)' : 'rotate(0deg)' }}>
          ▼
        </div>
      </button>
...
function WalletCard({
  ...
  return (
    <div className={`border-2 rounded-xl p-4 transition-all ${hasBalance ? 'border-[var(--success)] bg-[var(--success)]/10 shadow-[0_0_15px_var(--success)]' : 'border-gray-600 bg-[#1a1a3e]'}`}>
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-bold text-[var(--accent)]">#{index}</span>
            {hasBalance && <span className="text-xs bg-[var(--success)] text-black px-2 py-0.5 rounded font-bold uppercase">Funded</span>}
          </div>
      ...
      <div className="flex items-center justify-between mb-3 p-3 bg-[#0f0f23] rounded-lg border border-[var(--primary)]/30">
        <div>
          <p className="text-xs text-[var(--secondary)] font-bold uppercase">Balance</p>
